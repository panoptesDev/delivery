...
const (
	...
	DefaultPanoRPCUrl = "http://localhost:8545"
...
type Configuration struct {
	...
	PanoRPCUrl       string `mapstructure:"pano_rpc_url"`       // RPC endpoint for pano chain
...
func InitDeliveryConfigWith(homeDir string, deliveryConfigFilePath string) {
	...
	if strings.Compare(conf.PanoRPCUrl, "") != 0 {
		return
	}
	...
	if maticRPCClient, err = rpc.Dial(conf.PanoRPCUrl); err != nil {
		log.Fatal(err)
	}
